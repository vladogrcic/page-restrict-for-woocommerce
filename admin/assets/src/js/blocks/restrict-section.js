...
const icon = <svg><image href= {page_restrict_wc.sidebar_img}/></svg>;
...
registerBlockType('page-restrict-wc/restrict-section', {
	...
	edit: function (props) {
		...
		var initAttributes = page_restrict_wc.attributes ? page_restrict_wc.attributes : false;
				...
				for (let i = 0; i < sectionBlocks.length; i++) {
					if (props.clientId === sectionBlocks[i].clientId) {
						if (typeof sectionBlocks[i + next] !== "undefined") {
							...
							attributesIn.notAllProductsRequired 	= sectionBlocks[i + next].attributes.notAllProductsRequired;
						}
						else {
		...
		return <div
			...
			>
				...
				<InspectorControls>
					...
					>
						<label>{__('Show restricted content', 'page_restrict_domain')}</label>
						<br/>
						<br/>
						<ToggleControl
						...
						>
							<span>{__("Show content if user has no access.", 'page_restrict_domain')}</span>
							<br/>
							<span>{__("This is to notify the user they don't have access and what to do to access it.", 'page_restrict_domain')}</span>
						</span>
						<br/>
						<span
						...
						<hr/>
						<label>
							{__('Copy settings from another block', 'page_restrict_domain')}
						</label>
						<br/>
						<br/>
						<ToggleControl
						...
						<div>
							{
								prwc_termNames.length?
									...
									/>
								:
								<label>{__('No available products to show', 'page_restrict_domain')}</label>
							}
							{disabledInput?
							<Disabled>
								<ToggleControl

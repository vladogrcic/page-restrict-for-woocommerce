...
class EditFunc {
	...
	static changeOtherBlocksValuesNonActive (side, attributesIn, disable, disableNABNotice) {
		var next = 0;
		if(side === 'above'){
			next = -1;
		}
	...
	changeAllBlocks(){
		...
		for (var j = 0; j < sectionBlocksGen.length; j++) {
			sectionBlocksGen[j].attributes = this.constructor.changeOtherBlocksValuesNonActive('above', sectionBlocksGen[j].attributes, false, false);
			sectionBlocksGen[j].attributes = this.constructor.changeOtherBlocksValuesNonActive('below', sectionBlocksGen[j].attributes, false, false);
		}
	}

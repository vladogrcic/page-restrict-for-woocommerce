			...
			if(get_option('woocommerce_enable_guest_checkout') === 'yes'){
				add_action( 'admin_notices', function () use ($plugin_title) {
					...
					<div class="notice notice-error">
						...
						<a href="<?php echo admin_url( 'admin.php?page=wc-settings&tab=account', 'https' ); ?>"><?php esc_html_e( 'WooCommerce --> Settings --> Accounts & Privacy', 'page_restrict_domain' ); ?></a>
						<p><?php esc_html_e( 'Disable option: "Allow customers to place orders without an account".', 'page_restrict_domain' ); ?></p>
					</div>
					<?php
				});
			}
